...
@Component({
  selector: 'app-test-model-input-result',
  imports: [],
  template:`
    <h1>Contacts List</h1>
    <ul>
...
export class TestModelInputResult {
  searchTermInput = input('');
  ...
  filteredContacts = computed(() => {
    ...
    //Filter list
    return this.contacts().filter(c => c.name.toLowerCase().includes(this.searchTermInput().toLowerCase()));
